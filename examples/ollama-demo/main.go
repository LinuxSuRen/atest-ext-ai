...
func main() {
	...
	msgs = append(msgs, llms.TextParts(llms.ChatMessageTypeHuman, fmt.Sprintf("get all table names, then return the table similar to %s", "user")))
...
func dispatchCall(c *Call) (llms.MessageContent, bool) {
	...
	case "getAllTables":
		...
		if err == nil {
			tables["tables"] = fmt.Sprintf("%v", data.Meta.Tables)
		}

		b, err := json.Marshal(tables)
		if err != nil {
			return llms.TextParts(llms.ChatMessageTypeTool, ""), true
		}
		return llms.TextParts(llms.ChatMessageTypeTool, string(b)), false
	default:
